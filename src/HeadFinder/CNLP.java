...
public class CNLP {
	...
	public static void main(String[] args) throws IOException{
		...
		String fileLocation = new String("C:\\Users\\spimi\\workspace\\Thesis\\Data\\QuestionClassification\\train_5500.label.txt");//
	...
	public static String[] extractHeadWord(List<String> questionList, List<String> posList, String question, Tree tree, SemanticHeadFinder headFinder) {
		...
		if(questionList.get(0).equals("What")){
			for(String condition:checkRegex(question)){
				if(!condition.equals("HUM:desc")){
	 ...
	 */
	public static List<String> checkRegex(String question){
		String condition = "";
		List<String> returnList = new ArrayList<String>();
		String descDef1 = "W|what (is|are)( (a|an|the))?( \\w+)( \\w+)?.?.?";
		String descDef2 = "W|what (do|does) .+ mean.?.?";
		String entySubs = "W|what is|are .+ (composed of|made out of).?.?";
		String descDesc = "W|what does .+ do.?.?";
		String entyTerm = "W|what do you call .+";
		String descReas1 = "W|what (causes|cause).*";
		String descReas2 = "W|what (is|are) .+ used for.?.?";
		String abbrExp = "W|what (does|do) .+ stand for.?.?";
		String humDesc = "W|who (is|was) [A-Z]\\w+.?.?";
		...
		for (int i = 0; i < conditions.length; i++) {
			Pattern pattern = Pattern.compile(conditions[i]);
	...
	public void writeData(List<String[]> featureList) throws FileNotFoundException, UnsupportedEncodingException{
		PrintWriter writer = new PrintWriter("C:\\Users\\spimi\\workspace\\Thesis\\src\\features.txt", "UTF-8");
